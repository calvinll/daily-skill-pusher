...
该命令会生成 `dist-web/`，供 GitHub Pages 发布。
...
- 官方 changelog / what’s-new 提供的“新近值得关注”信号
- 最近推送历史与主题轮换
- related skills 跟进推荐
...
为了降低 GitHub 定时偶发漏触发的风险，工作流现在使用：
- 主触发：北京时间 09:05
- 备份触发：北京时间 10:05

如果主触发已经推送成功，备份触发会被应用层的“当天已推送”逻辑自动跳过。
...
- GitHub Actions 的 cron 使用 UTC，当前主触发 `5 1 * * *` 对应北京时间 09:05，备份触发 `5 2 * * *` 对应北京时间 10:05。
...
- `/history` 历史推荐
- `/skills` 技能列表
- `/skill.html?name=<skill>` 技能详情页
- `/api/today` / `/api/history` / `/api/skills` / `/api/skills/:name` API

为了让更多人直接访问，项目还支持：
- `pnpm build:web` 生成静态产物 `dist-web/`
- `.github/workflows/deploy-pages.yml` 自动部署到 GitHub Pages
- 静态站点已适配 GitHub Pages 的仓库子路径（如 `/daily-skill-pusher/`）
